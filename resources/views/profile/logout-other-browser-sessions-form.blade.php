<x-action-section>
    ...
    <x-slot name="content">
        ...
        <div>
            {{ __('إذا لزم الأمر، يمكنك تسجيل الخروج من جميع جلسات المتصفح الأخرى عبر جميع أجهزتك. يتم إدراج بعض جلساتك الأخيرة أدناه؛ ومع ذلك، قد لا تكون هذه القائمة شاملة. إذا كنت تشعر أن حسابك قد تعرض للاختراق، فيجب عليك أيضًا تحديث كلمة المرور الخاصة بك.') }}
        </div>
                            ...
                            <div>
                                <div class="small font-weight-lighter text-muted">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-success font-weight-bold">{{ __('هذا الجهاز') }}</span>
                                    @else
        ...
        <x-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                ...
                <div class="mt-3 w-md-75" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-input type="password" placeholder="{{ __('كلمة المرور') }}"
                                 x-ref="password"
